...
void loop() {
  ...
  if (now - lastMsg > 5000) {
    ...
    char tempCStr[8], tempFStr[8], humStr[8];
    dtostrf(tempC, 1, 2, tempCStr);
    dtostrf(tempF, 1, 2, tempFStr);
    dtostrf(hum, 1, 2, humStr);

    String tempCMessage = "Temp: " + String(tempCStr) + " C";
    String tempFMessage = "Temp: " + String(tempFStr) + " F";
    String humMessage   = "Humidity: " + String(humStr) + " %";

    client.publish("sensor/temp_c", tempCMessage.c_str());
    client.publish("sensor/temp_f", tempFMessage.c_str());
